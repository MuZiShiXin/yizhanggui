...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        _line.backgroundColor = [UIColor hx_colorWithHexRGBAString:kBeijingColor];
...
- (void)moveNode:(CGFloat)indentationX{
    ...
    CGRect frame1 = CGRectMake(0, (cellHeight-10)/2, 10, 10);
    frame1.origin.x = indentationX+15;
    _leftImage.frame = frame1;
    
    CGRect frame = CGRectMake(leftMargin-10, 0, cellWidth-leftMargin, cellHeight);
    frame.origin.x = leftMargin+indentationX;
    _nodeLabel.frame = frame;
    
    CGRect frame2 = CGRectMake(0, cellHeight-1, cellWidth, 1);
    frame2.origin.x = indentationX+15;
    _line.frame = frame2;
}
@end
...
- (void)addFirstLoadNodes{
    ...
    for (int i = 0 ; i<_nodes.count;i++) {
        ...
        if (node.isRoot) {
            ...
            if (node.isExpand) {
                [self expandNodesForParentID:node.workTypeId insertIndex:[_tempNodes indexOfObject:node]];
            }
        }
    }
...
- (void)judgeLeafAndRootNodes{
    for (int i = 0 ; i<_nodes.count;i++) {
        ...
        for (YKNodeModel *tempNode in _nodes) {
            if ([tempNode.fatherWorkTypeId isEqualToString:node.workTypeId]) {
                isLeaf = NO;
            }
            if ([tempNode.workTypeId isEqualToString:node.fatherWorkTypeId]) {
                isRoot = NO;
            }
            if (!isRoot && !isLeaf) {
                break;
...
- (void)setDepth:(NSUInteger)level parentIDs:(NSArray*)parentIDs childrenNodes:(NSMutableArray*)childrenNodes{
    ...
    for (YKNodeModel *node in childrenNodes) {
        if ([parentIDs containsObject:node.fatherWorkTypeId]) {
            node.level = level;
            [leftNodes removeObject:node];
            [newParentIDs addObject:node.workTypeId];
        }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (currentNode.isExpand) {
        //expand
        [self expandNodesForParentID:currentNode.workTypeId insertIndex:indexPath.row];
...
- (NSUInteger)expandNodesForParentID:(NSString*)parentID insertIndex:(NSUInteger)insertIndex{
    ...
    for (int i = 0 ; i<_nodes.count;i++) {
        YKNodeModel *node = _nodes[i];
        if ([node.fatherWorkTypeId isEqualToString:parentID]) {
            ...
            if (node.isExpand) {
               insertIndex = [self expandNodesForParentID:node.workTypeId insertIndex:insertIndex];
            }
        }
    }
