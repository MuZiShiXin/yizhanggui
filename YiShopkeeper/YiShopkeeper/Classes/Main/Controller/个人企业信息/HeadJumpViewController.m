...
- (void)buttonClicked:(UIButton *)sender {
    ...
    TJLActionSheet *sheet = [[TJLActionSheet alloc] initWithTitle:nil buttonTitles:@[@"拍照", @"从相册选择"] redButtonIndex:2 clicked:^(NSInteger buttonIndex) {
        if (buttonIndex == 0) {
            ...
            [[TJLImagePickerController sharedInstance] showPickerInController:self total:[self.tupianshu isEqualToString:@"4"]?4:1  successBlock:^(NSArray *imageArray) {
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 64, kScreenWidth, kScreenHeight / 2.0) collectionViewLayout:layout];
